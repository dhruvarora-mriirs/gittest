...
public class UserControllerTest {
	...
	public void findByUserName() throws Exception {
		...
		mockMvc.perform(get("/api/v1/user/username/{userName}", user.getUserName())).andExpect(status().isOk())
				.andExpect(content().contentType(MediaType.APPLICATION_JSON)).andExpect(jsonPath("$.id", is(1)))
				.andExpect(jsonPath("$.userName", is("dhruv"))).andExpect(jsonPath("$.gender", is("male")))
				.andExpect(jsonPath("$.dob", is("1970-01-01T00:00:01.968+0000")))
				.andExpect(jsonPath("$.phone", is("912210101"))).andExpect(jsonPath("$.emailId", is("dhruv@gmail.com")))
				.andDo(print());
	...
	public void addNewUserTest() throws Exception {
		String json = "{\"id\":2,\"userName\":\"dhruv\",\"gender\":\"male\",\"dob\":1968,\"phone\":\"912210101\",\"emailId\":\"dhruv@gmail.com\"}";
//	  String json = "{id:1,userName:\"dhruv\",gender:\"male\",dob:\"1970-01-01T00:00:01.968+0000\",phone:\"912210101\",emailId:\"dhruv@gmail.com\"}";
		...
		MvcResult result = mockMvc
				.perform(MockMvcRequestBuilders.post("/api/v1/user").content(json)
	...
	public void deleteUserAPI() throws Exception {
		...
		when(userService.getUser(1)).thenReturn(Optional.of(user1));

		mockMvc.perform(MockMvcRequestBuilders.delete("/api/v1/user/{id}", user1.getId()))
