...
public class UserService {
	...
	public List<User> getMaleUser() {
		...
		// sorted by userName in descending and where gender=male;
	...
	public List<CustomResponse> getNoOfUser() // count based on custom group
	{
		...
		aggregate = newAggregation(group("userName", "gender").count().as("noOfUsers"),
				sort(Sort.Direction.DESC, "noOfUsers"));

		AggregationResults<CustomResponse> groupResults = mongoTemplate.aggregate(aggregate, User.class,
	...
	public AggregatorResponse getMaxUserId() {
		...
		GroupOperation groupFirstAndLast = group().first("_id").as("maxUID").first("userCount").as("maxUIDCount")
				.last("_id").as("minUID").last("userCount").as("minUIDCount");

		Aggregation aggregation = newAggregation(sumUID, sortByCount, groupFirstAndLast);

		AggregationResults<AggregatorResponse> result = mongoTemplate.aggregate(aggregation, User.class,
				AggregatorResponse.class);
